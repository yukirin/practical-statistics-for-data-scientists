...
# Compute the mean, trimmed mean, and median for Population. For `mean` and `median`
# we can use the _pandas_ methods
# of the data frame. The trimmed mean requires the `trim_mean` function in _scipy.stats_.
...
# The `cut` method for _pandas_ data splits the dataset into bins. There are a number of arguments
# for the method. The following code creates equal sized bins.
...
# Density Estimates
# Density is an alternative to histograms that can provide more insight
# into the distribution of the data points.
...
# is cumbersome and
# it's more convenient to plot the correlation as a heatmap. The _seaborn_ package provides
# a convenient implementation for heatmaps.
...
# The above heatmap works when you have color. For the greyscale images, as used in the book,
# we need to visualize the direction as well. The following code shows
# the strength of the correlation using ellipses.
...
# Scatterplots
# Simple scatterplots are supported by _pandas_. Specifying the marker as `$\u25EF$`
...
# Here methods that visualize densities are more useful.
# The `hexbin` method for _pandas_ data frames is one powerful approach.
